...
void _update_solution_mysql(int solution_id, int result, int time, int memory,
	...
	if (oi_mode) {
		sprintf(sql,
				"UPDATE solutions SET result=%d,time=%d,memory=%d,pass_rate=%f,judger='%s' WHERE id=%d LIMIT 1%c",
				result, time, memory, pass_rate,http_username, solution_id, 0);
	} else {
		sprintf(sql,
				"UPDATE solutions SET result=%d,time=%d,memory=%d,judger='%s' WHERE id=%d LIMIT 1%c",
...
void _addceinfo_mysql(int solution_id) {
	...
	snprintf(sql, (1 << 16) - 1, "DELETE FROM compile_infos WHERE solution_id=%d",
			solution_id);
	...
	strcpy(end, "INSERT INTO compile_infos VALUES(");
	end += strlen(sql);
	*end++ = '\'';
...
void _addreinfo_mysql(int solution_id, const char * filename) {
	...
	snprintf(sql, (1 << 16) - 1, "DELETE FROM runtime_infos WHERE solution_id=%d",
			solution_id);
	mysql_real_query(conn, sql, strlen(sql));
	rend = reinfo;
	...
	strcpy(end, "INSERT INTO runtime_infos VALUES(");
	end += strlen(sql);
	*end++ = '\'';
	end += sprintf(end, "%d", solution_id);
	*end++ = '\'';
...
void _update_user_mysql(char * user_id) {
	char sql[BUFFER_SIZE];
	sprintf(sql,
			"UPDATE `users` SET `solved`=(SELECT count(DISTINCT `problem_id`) FROM `solutions` WHERE `user_id`=\'%s\' AND `result`=\'4\') WHERE `id`=\'%s\'",
			user_id, user_id);
	if (mysql_real_query(conn, sql, strlen(sql)))
		write_log(mysql_error(conn));
	sprintf(sql,
			"UPDATE `users` SET `submit`=(SELECT count(*) FROM `solutions` WHERE `user_id`=\'%s\' and problem_id>0) WHERE `id`=\'%s\'",
			user_id, user_id);
	if (mysql_real_query(conn, sql, strlen(sql)))
		write_log(mysql_error(conn));
}
...
void _update_problem_mysql(int p_id) {
	char sql[BUFFER_SIZE];
	sprintf(sql,
			"UPDATE `problems` SET `accepted`=(SELECT count(*) FROM `solutions` WHERE `problem_id`=\'%d\' AND `result`=\'4\') WHERE `id`=\'%d\'",
			p_id, p_id);
	if (mysql_real_query(conn, sql, strlen(sql)))
		write_log(mysql_error(conn));
	sprintf(sql,
			"UPDATE `problems` SET `submit`=(SELECT count(*) FROM `solutions` WHERE `problem_id`=\'%d\') WHERE `id`=\'%d\'",
			p_id, p_id);
	if (mysql_real_query(conn, sql, strlen(sql)))
		write_log(mysql_error(conn));
}
...
void _get_solution_mysql(int solution_id, char * work_dir, int lang) {
	...
	MYSQL_RES *res;
	MYSQL_ROW row;
	sprintf(sql, "SELECT source FROM source_codes WHERE solution_id=%d",
			solution_id);
	mysql_real_query(conn, sql, strlen(sql));
...
void _get_solution_info_mysql(int solution_id, int & p_id, char * user_id,
	...
	sprintf(sql,
			"SELECT problem_id, user_id, language FROM solutions where id=%d",
			solution_id);
	//printf("%s\n",sql);
	mysql_real_query(conn, sql, strlen(sql));
	res = mysql_store_result(conn);
...
void _get_problem_info_mysql(int p_id, int & time_lmt, int & mem_lmt,
	...
	MYSQL_ROW row;
	sprintf(sql,
			"SELECT time_limit,memory_limit,spj FROM problems where id=%d",
			p_id);
	mysql_real_query(conn, sql, strlen(sql));
	res = mysql_store_result(conn);
	row = mysql_fetch_row(res);
