...
void init_mysql_conf() {
	...
	//check the line below,changed as absolute directory.
	...
	if (fp != NULL) {
		...
		sprintf(query,
				"SELECT id FROM solutions WHERE language in (%s) and result<2 and MOD(id,%d)=%d ORDER BY result ASC,id ASC limit %d",
...
bool _check_out_mysql(int solution_id, int result) {
	...
	sprintf(sql,
			"UPDATE solutions SET result=%d,time=0,memory=0,judgetime=NOW() WHERE id=%d and result<2 LIMIT 1",
			result, solution_id);
	if (mysql_real_query(conn, sql, strlen(sql))) {
